#!/usr/bin/env python3
"""Generate visual outputs for Task 1.2.1 card detection."""
...
from modules.card_detector import save_detection_preview
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create card-detection visual outputs for Task 1.2.1.")
    parser.add_argument(
        "--samples-dir",
        type=Path,
    ...
    for sample_path in sample_images:
        ...
        paths = save_detection_preview(sample_path, sample_output_dir)
        print(f"{sample_path.name} -> {sample_output_dir}")
        print(f"  detection: {paths['detection'].name}")
