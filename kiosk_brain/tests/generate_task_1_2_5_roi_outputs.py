#!/usr/bin/env python3
"""Generate ROI-cropped outputs for Task 1.2.5."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Relative ROI as x,y,w,h (fractions). If omitted uses config.ROI.default",
    )
    args = parser.parse_args()

    roi_rel = None
    if args.roi is not None:
        roi_rel = tuple(float(x) for x in args.roi.split(","))
    ...
    for sample_path in sample_images:
        ...
        paths = save_roi_preview(sample_path, sample_output_dir, roi_rel=roi_rel)
        print(f"{sample_path.name} -> {sample_output_dir}")
        print(f"  roi: {paths['roi'].name}")
