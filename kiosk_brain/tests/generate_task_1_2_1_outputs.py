#!/usr/bin/env python3
"""Generate visual outputs for Task 1.2.1 grayscale conversion."""
...
from modules.ocr import save_grayscale_preview
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create grayscale visual outputs for Task 1.2.1."
    )
    parser.add_argument(
        "--samples-dir",
        type=Path,
    ...
    for sample_path in sample_images:
        ...
        paths = save_grayscale_preview(sample_path, sample_output_dir)
        print(f"{sample_path.name} -> {sample_output_dir}")
        print(f"  preview: {paths['preview'].name}")
        print(f"  grayscale: {paths['grayscale'].name}")
