#!/usr/bin/env python3
"""Generate visual outputs for Task 1.2.2 adaptive thresholding."""
...
from modules.ocr import save_threshold_preview


def iter_sample_images(samples_dir: Path) -> list[Path]:
    """Return sample image files in a stable order."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create adaptive threshold visual outputs for Task 1.2.2."
    )
    parser.add_argument(
        "--samples-dir",
        type=Path,
    ...
    for sample_path in sample_images:
        ...
        paths = save_threshold_preview(sample_path, sample_output_dir)
        print(f"{sample_path.name} -> {sample_output_dir}")
        print(f"  preview: {paths['preview'].name}")
        print(f"  threshold: {paths['threshold'].name}")
